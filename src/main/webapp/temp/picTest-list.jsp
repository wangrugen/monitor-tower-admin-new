...
<html>
  ...
  <body class="layui-anim layui-anim-up">
    ...
    <div class="x-body">
      ...
      <xblock>
        <button class="layui-btn" onclick="x_admin_show('启动图片接收服务','./pic-test-port.jsp',500,250)"><i class="layui-icon"></i>开始启动</button>
      </xblock>
      ...
      layui.use('laydate', function(){
        ...
        layui.use('table', function(){
            ...
            table.render({
                ...
                ,width: 1500
                ,height: 312
                ,url: '${contextPath}/pic/testPicList' //数据接口
                ,page: true //开启分页
                ,cols: [[ //表头
                    {field: 'ID', title: 'ID', width:80, sort: true, fixed: 'left'}
                    ,{field: 'IMIE', title: 'IMIE', width:80}
                    ,{field: 'VERSION', title: '版本', width:80, sort: true}
                    ,{field: 'JIAO_DU', title: '角度', width:80}
                    ,{field: 'WEI_YI', title: '位移', width: 177}
                    ,{field: 'ZHOU_QI', title: '周期', width: 80, sort: true}
                    ,{field: 'GUAN_ZHU_DIAN', title: '是否报警', width: 80, sort: true,templet:function (d){
                            if (d.GUAN_ZHU_DIAN=='0'){ return '正常'}else { return '报警'}
                        }}
                    ,{field: 'JING_DU', title: '经度', width: 80}
                    ,{field: 'WEI_DU', title: '纬度', width: 135, sort: true}
                    ,{field: 'IMAGE_URL', title: '图片预览',width: 135, templet:function (row){
                        if(row!=null&&row.IMAGE_URL!=null&&row.IMAGE_URL!='')
                        return '<div><img src="${contextPath}'+row.IMAGE_URL+'" onclick="openImage(\'${contextPath}'+row.IMAGE_URL+'\')"></div>'
                        else '图片未知请检查'
                        }}
